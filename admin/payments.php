        ...
        <div class="col-lg-10 col-12 p-3">
            ...
            <div class="card">
              ...
              <table class="table table-striped table-hover table-bordered text-center">
                ...
                <tbody>
                    <?php 
                    $query_loans = mysqli_query($conn, "SELECT * FROM loans l INNER JOIN balance b ON l.USER_ID=b.USER_ID");
                 
                      if (isset($_POST['showBtn'])) {
                            $show = $_POST['show'];

                            $query_loans = mysqli_query($conn, "SELECT * FROM loans l INNER JOIN balance b ON l.USER_ID=b.USER_ID LIMIT $show ");
                           
                      }

                      if (isset($_POST['searchBtn'])) {
                            $search = $_POST['search'];
                            $query_loans = mysqli_query($conn, "SELECT * FROM loans l INNER JOIN balance b ON l.USER_ID=b.USER_ID WHERE l.FNAME LIKE '%$search%' OR l.LNAME LIKE '%$search%' OR l.MIDDLE LIKE '%$search%' OR b.TOTAL_BALANCE LIKE '%$search%' OR l.ID LIKE '%$search%' ");
                        ...
                        while ($data = mysqli_fetch_assoc($query_loans)) {
                            if ($data['STATUS'] == "active") {
                                ...
                                <tr>
                                    ...
                                    <td><a href="add_payment.php?ID=<?php echo $data['USER_ID'] ?>" class="btn btn-primary">Add</a></td>
                                </tr>
                            <?php
                            }
                        }
